...
internal object BitmapUtils {
    ...
    fun writeTempStateStoreBitmap(context: Context, bitmap: Bitmap, uri: Uri?): Uri? {
        ...
        try {
            ...
            } else if (uri.path?.let { File(it).exists() } == true) {
                needSave = false
            }
            if (needSave) {
    ...
    fun writeBitmapToUri(
        ...
        try {
            outputStream = context.contentResolver.openOutputStream(uri ?: Uri.EMPTY)
    ...
    private fun cropBitmap(
        ...
        try {
            ...
            val fullBitmap = decodeImage(context.getContentResolver(), loadedImageUri, options)
            if (fullBitmap != null) {
                try {
    ...
    private fun rotateAndFlipBitmapInt(
        ...
        if ((degrees > 0) || flipHorizontally || flipVertically) {
            ...
            matrix.postScale(if (flipHorizontally) -1f else 1f, if (flipVertically) -1f else 1f)
            val newBitmap =
    ...
    private val maxTextureSize: Int
        private get() {
            ...
            try {
                ...
                val configurationsList: Array<EGLConfig?> = arrayOfNulls<EGLConfig>(
    ...
    internal class BitmapSampled(mBitmap: Bitmap?, sampleSize: Int) {
        /** The bitmap instance  */
        val bitmap: Bitmap?
    ...
    internal class RotateBitmapResult(bitmap: Bitmap?, degrees: Int) {
        /** The loaded bitmap  */
        val bitmap: Bitmap?
